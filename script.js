// Declaring global variables and initializing
let num1 = 0;
let tempNum1Array = [];
let num2 = 0;
let tempNum2Array = [];
...
function divide(num1, num2) {
  ...
  if (divideValue == "Infinity") {
    return alert("Zero no likey! CRITICAL FAILURE! Zero hates being divided by!");
  } else {
...
// Operator variable to determine which function to call
function operate(operator, num1, num2) {
  switch (operator) {
    ...
    case "/":
      ...
      break;
    default:
      alert("ERROR! No operator recieved or unknown type!");
  }
}
...
function runCalculator() {
  ...
  function getOperatorSelection() {
    ...
    operatorButtons.forEach((button) => {
      button.addEventListener('click', () => {
        ...
        // check to see if a multi-operand string exists and needs to be calculated on the fly
        if ((tempNum1Array != "") && (tempNum2Array != "")) {
          const calcDisplay = document.querySelector("#display");
          num1 = tempNum1Array.join("");
          num2 = tempNum1Array.join("");
  ...
  const decimalButton = document.querySelector(".decimal");
    decimalButton.addEventListener('click', () => {
      ...
      const calcDisplay = document.querySelector("#display");
        ...
        console.log("append the array2 with decimal: ", tempNum2Array);
        num2 = tempNum2Array.join("");
        calcDisplay.textContent = num2;
        document.getElementById("decimal").disabled = true;
  ...
  // Calling/running the advanced sub-functions
  getNumberClicked();
  getOperatorSelection();
  runCalculation();
}

// Calling/running the main function
runCalculator();
